...
| 3.3 | `/how-to-cook-puffball-mushrooms/` | Guide (species) | **Written 2026-08-31, hero pending** — 2,109 words, Recipe schema, backlinks from 4 pages, 2 new cook-time rows. Safety angle is the slice test (pure white, featureless interior), mirroring the morel hollow-check pattern. Committed locally; not deployed until `cookmushroom-puffball-hero.webp` exists. |
| 3.4 | ~~`/how-to-cook-hedgehog-mushrooms/`~~ → `/how-to-cut-mushrooms/` | Guide (technique) | **Replaced and written 2026-08-31, hero pending.** User chose the fluting swap on 2026-08-31; a keyword check then showed a better target again. `how to cut mushrooms` is **2,900/mo at SD 27** against fluting's 210/mo at SD 31, and the site had no cutting page — so fluting ships as a section inside it rather than as its own page, which would have blocked the head term later (rule 4). 1,890 words, 12-row cut-by-mushroom table, backlinks from 4 pages. |
